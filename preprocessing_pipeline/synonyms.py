...
class Synonyms:
            ...
            if w in self.synonym_to_dict:
                ...
                for syn in wordnet.synsets(w):
                    for lem in syn.lemmas():
                        w2 = lem.name().lower().replace('_', '$')
                        if w2 not in self.synonym_to_dict:
                            self.synonym_to_dict[w2] = w
        return new_d

    def old_load_synonyms(self, file):
        '''
        synonyms are saved in list form, because the list is very short.
        if this list grows by orders of magnitude,
    ...
    def old_replace_synonyms(self, d):
        '''
        This is carefully constructed to preserve uppercase and lowercase
         letters for POS tagging.
